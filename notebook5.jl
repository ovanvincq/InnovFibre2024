...
# ╔═╡ 7aa81ce0-240c-11ef-0067-4957bab5da8a
md"""
# Fibres microstructurées
## Fibre à cristal photonique
Soit une fibre constituée de trois couronnes de trous d'air (n=1) insérés dans la silice (n=1.45). Le pas du réseau est de 2 µm, le diamètre des trous est de 1.5 µm et la PML commence à 8 µm du centre de la fibre et possède une épaisseur de 2 µm.
"""

# ╔═╡ ade73c53-8612-4f0b-a782-fac1c353fae6
model = GmshDiscreteModel("PCF.msh");

# ╔═╡ 19d42078-1e30-4d3a-a4a0-7b0bb92268b9
md"Création du profil d'indice"

# ╔═╡ ccd184e8-7bd1-4fbd-9a38-8cd838d7b5ca
begin
	Pitch=2
	r_hole=0.75
	pos=ring.(1:3)
...
# ╔═╡ e31c0eef-65fe-44d6-b4ca-2f7c41b08ec8
begin
	x=-8:0.025:8
	contourf(x,x,sqrt.(eps_PCF.(tuple.(x,x')))',linewidth=0,levels=100,title="RIP",aspect_ratio=:equal,xlims=(minimum(x),maximum(x)),ylims=(minimum(x),maximum(x)))
end

# ╔═╡ 3879786f-6e69-4980-a840-b45cfd3e0f90
lambda=1.3;

# ╔═╡ f5d88fe1-a910-4d9a-b192-0d2591c92aca
md"Calcul de la valeur approximative de l'indice effectif"

# ╔═╡ b3fdda6c-6ce4-4bf2-a508-01e3cdf7dcc1
neff_approx=approx_neff_PCF(lambda,2*r_hole,Pitch)

# ╔═╡ be0a59d6-69e4-40b9-a6c8-deebe7079ab3
# ╠═╡ disabled = true
#=╠═╡
m=FEM2D(lambda,eps_PCF,model,neigs=4,approx_neff=neff_approx,field=true,solver=:MUMPS,type=:Vector,dPML=2)
  ╠═╡ =#

# ╔═╡ 53ffe64a-db00-4bef-b21e-b3f66ef41859
#=╠═╡
Px,Py,Pz=PoyntingVector(m[4]);
  ╠═╡ =#

# ╔═╡ 8f0ec34f-53fd-4b77-81bd-b6a614faec65
#=╠═╡
begin
	contourf(x,x,real(getValue(Pz,(Point.(x,x'))))',linewidth=0,levels=100,title="Pz",aspect_ratio=:equal,xlims=(minimum(x),maximum(x)),ylims=(minimum(x),maximum(x)))
end
  ╠═╡ =#

# ╔═╡ 55968a58-b92c-41d6-8809-d8f589f1168a
md"""
## Fibres à bandes interdites photoniques
Dans cet exemple, la gaine est un réseau triangulaire de barreaux circulaire de pas  10 µm. Le rayon des barreaux est 3 µm, leur indice de 1.47 et ils sont plongés dans un milieu d'indice 1.45. Pour déterminer les BIP, nous utiliserons un maillage périodique. Comme la cellule élementaire est fortement symétrique, le calcul des bandes sera restrient aux points de plus haute symétrie (Γ, M et K) de la zone de Brillouin irréductible.
"""

# ╔═╡ 0654c712-5f16-4ce5-bc44-76a84dcd6494
md"Calcul des vecteurs $\vec{k}_t$"

# ╔═╡ bcf22c45-483c-41a9-8363-bbd46fb73b4e
kt,weight=compute_kt(2,:Hexagon,Irreducible=true,MeshType=:Edge,Pitch=10);

# ╔═╡ 1c4a1cdc-1a7f-4b4c-80fa-708762ebab4b
md"Chargement du maillage périodique de la maille élémentaire"

# ╔═╡ cc129c0a-ee5f-4ec6-8bb2-2319be8e592d
model2 = GmshDiscreteModel("Cell_Hexagon2.msh");

# ╔═╡ 0c82d09f-1ce3-404f-8fee-152edf213db4
md"Définition de la permittivité"

# ╔═╡ 740f1f65-2a3a-408a-a820-edadb2987d62
epsilon_maille=x->(1.47-0.02*(hypot(x[1],x[2])>=3))^2;

# ╔═╡ 140a50fd-c66e-4870-89fe-4baf097446f3
md"Calcul de 30 bandes pour chaque vecteur $\vec{k}_t$ entre 0.8 et 1.6 µm"

# ╔═╡ 8e1c0baf-0390-4bf7-885f-eb02557ef452
# ╠═╡ disabled = true
#=╠═╡
begin
	vlambda=0.8:0.025:1.6
	mm=Matrix{Vector{Mode}}(undef,length(vlambda),length(kt))
	Threads.@threads for i in 1:length(kt)
    	mm[:,i]=FEM2D_periodic.(vlambda,epsilon_maille,Ref(model2),neigs=30,field=true,kt=kt[i]);
	end
end;
  ╠═╡ =#

# ╔═╡ 8139f593-e7af-4b4b-b114-5bdbf441273d
#=╠═╡
begin
	neff=zeros(length(vlambda),length(kt),30)
	P=plot()
	a=palette([:red,:green,:blue],length(kt))
	plot!(P,vlambda,1.45*ones(length(vlambda)),color=:black,label="")
	for i=1:30
    	neff[:,:,i]=[real(m[i].neff) for m in mm]
    	for k=1:length(kt)
        	plot!(P,vlambda,neff[:,k,i],color=a[k],label="")
    	end
	end
...
# ╔═╡ d20b9ccc-43e2-4406-82ce-675a5ff736cd
md"Nous allons maintenant déterminer le mode de coeur entre 0.925 et 1.115 µm lorsqu'il y a 4 couronnes de barreaux"

# ╔═╡ 1f8e1a45-923b-4ad3-94f3-75c7cdf51d19
...
# ╔═╡ 76d6f0c4-a6e4-4add-8fb4-aa0e09a68491
md"On charge le maillage"

# ╔═╡ c4a8b458-1087-4753-8d29-85270998cb76
model_PBG = GmshDiscreteModel("PBG.msh");

# ╔═╡ 70e39255-175b-401b-834f-93c3c97a78dc
md"On définit la permittivité"

# ╔═╡ 3ab97653-8d2d-4c44-b6b4-241674e45fe4
begin
	Pitch_PBG=10
	r_PBG=3
	pos_PBG=ring.(1:4)
	xc_PBG=vcat(first.(pos_PBG)...)*Pitch_PBG
	yc_PBG=vcat(last.(pos_PBG)...)*Pitch_PBG
	eps_PBG(x) = (any(@. hypot(x[1]-xc_PBG,x[2]-yc_PBG)<r_PBG)) ? 1.47^2 : 1.45^2
end;

# ╔═╡ 88be405e-dae5-40e3-9b75-2e066878c3f4
begin
	x_PBG=-50:0.1:50
	contourf(x_PBG,x_PBG,sqrt.(eps_PBG.(tuple.(x_PBG,x_PBG')))',linewidth=0,levels=100,title="RIP",aspect_ratio=:equal,xlims=(minimum(x_PBG),maximum(x_PBG)),ylims=(minimum(x_PBG),maximum(x_PBG)))
end

# ╔═╡ 425d3e7a-85c2-4cde-a5f5-e53850e05831
md"On calcule les modes"

# ╔═╡ f9ce6adb-e9f7-48b3-9cc8-2a63a698a4dd
#=╠═╡
begin
...
# ╔═╡ eb4e538e-06bd-4aab-b362-8a7c1ca66d52
#=╠═╡
begin
	mode_PBG2=vcat(mode_PBG...)
	neff_PBG=getproperty.(mode_PBG2,:neff)
	Aeff_PBG=Aeff.(mode_PBG2)
	...
	losses_PBG=losses.(mode_PBG2)*1E6
...
# ╔═╡ c52a3f2c-5780-4866-a785-314d48709ef8
md"""
## Fibre de Bragg
Nous allons étudier une fibre de Bragg constituée de 3 anneaux d'épaisseur 2 µm séparés de 4 µm avec un coeur de rayon 25 µm. L'indice des anneaux est 1.47 et celui de la silice 1.45.
"""

# ╔═╡ 3023fb51-f1b8-466e-a465-f28515366e33
md"Cut-off des modes de guide plan :"

# ╔═╡ aad8dc28-0f9c-4d94-955a-34e5f9bfdcca
2*2*sqrt(1.47^2-1.45^2)./(1:4)

# ╔═╡ 3479ec01-0a2d-417b-9dc9-3892eb59c4e2
md"Définition du profil de permittivité :"

# ╔═╡ 514743fd-2b6f-4ce8-8b5d-f387a449208d
epsilon_bragg=x->(1.45+0.02*(x[1]>25)-0.02*(x[1]>27)+0.02*(x[1]>31)-0.02*(x[1]>33)+0.02*(x[1]>37)-0.02*(x[1]>39))^2;

# ╔═╡ 93b3bc79-80f3-48ec-9fe8-211c11239db4
begin
	x_bragg=0:0.01:50
	plot(x_bragg,sqrt.(epsilon_bragg.(x_bragg)),ylabel="Indice de réfraction",xlabel="r (µm)",label="",title="RIP")
end

# ╔═╡ b84b1b73-091b-45ff-99f0-60f3d1315753
model_bragg = CartesianDiscreteModel((0,50),5000);

# ╔═╡ f658d270-facd-4b41-bfe1-c5e109dcee80
lambda_bragg=0.5:0.01:0.8;

# ╔═╡ 2d77fed7-4027-44d9-ab4d-7b1c50c4a53b
neff_approx_bragg=sqrt.((1.45^2).-(2.405/2/pi/25*lambda_bragg).^2);

# ╔═╡ b9e2613b-8f38-4e1f-9d58-9a9c6e176ecf
begin
	m1D_bragg=Vector{Vector{Mode}}(undef,length(lambda_bragg))
	for i=1:length(lambda_bragg)
	m1D_bragg[i]=FEM1D(lambda_bragg[i],0,epsilon_bragg,model_bragg,dPML=5,neigs=1,approx_neff=neff_approx_bragg[i])
	end
end

# ╔═╡ 30fd81f1-eafe-4dc8-8dab-7dc240ef9932
begin
	m1D_bragg2=vcat(m1D_bragg...)
	pertes=losses.(m1D_bragg2)*1E6
	plot(lambda_bragg,pertes,yaxis=:log,xlabel="Longueur d'onde (µm)",ylabel="Pertes (dB/km)",label="")
end

# ╔═╡ cea5541f-1fe0-4bb9-8adb-0057a4d26740
md"Calcul 2D pour les pertes par courbures pour la longueur d'onde lambda_bragg[11]=0.6 µm"

# ╔═╡ a9b138a9-9573-4bfc-bef9-33404374d858
epsilon_bragg2D=x->epsilon_bragg(hypot(x[1],x[2]));

# ╔═╡ d2a7a278-9e14-43b7-a7e7-b9ec1876451f
Rc=150; #en mm

# ╔═╡ 42f06760-ab50-4c2f-8d92-96a52cbd97ee
epsilon_bragg2D_bent=x->epsilon_bragg2D(x)*(1+x[1]/Rc/1000)^2;

# ╔═╡ cc21fcbf-8e97-4977-8f20-21b18097b4f0
model_bragg2D = GmshDiscreteModel("Bragg.msh");

# ╔═╡ acc11d1c-a19c-46f8-ab3b-210bae98477a
# ╠═╡ disabled = true
#=╠═╡
m2D_bragg=FEM2D(lambda_bragg[11],epsilon_bragg2D_bent,model_bragg2D,neigs=10,approx_neff=neff_approx_bragg[11],field=true,solver=:MUMPS,dPML=5)
  ╠═╡ =#

# ╔═╡ 26c10a9c-3942-4f30-a286-a60783f20de0
md"Numéro du mode à visualiser"

# ╔═╡ c9ed05b6-ddb1-4667-a9c0-bc7e22cd9969
#=╠═╡
@bind nmode NumberField(1:length(m2D_bragg))
  ╠═╡ =#

# ╔═╡ 82adfab6-04e3-4b75-ad15-14665002f8d8
#=╠═╡
begin
	x_bragg2=-50:0.25:50
	contourf(x_bragg2,x_bragg2,real(getValue(abs2(m2D_bragg[nmode].field.E),(Point.(x_bragg2,x_bragg2'))))',linewidth=0,levels=100,title="|E|² - $(round(losses(m2D_bragg[nmode])*1E9)/1000) dB/km",aspect_ratio=:equal,xlims=(minimum(x_bragg2),maximum(x_bragg2)),ylims=(minimum(x_bragg2),maximum(x_bragg2)))
end
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "761b295633b34009a5ed6cd97d333114c4d06794"
