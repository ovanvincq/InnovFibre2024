...
using Plots,OpticalFibers, Gridap,GridapGmsh,PlutoUI
...
md"""
# Couplage et propagation
A titre d'exemple, nous utiliserons une fibre à profil parabolique avec un coeur de rayon 5 µm et d'indice 1.48 et une gaine d'indice 1.45.
"""

# ╔═╡ 5540beed-102f-46a7-bc65-29bd33b0138e
md"## Calcul des modes"

# ╔═╡ 24fbe89b-2134-415c-831f-255f5cdf0d93
md"Calcul en 1D"
...
md"Le nombre de modes est bien le même en 1D et en 2D"

# ╔═╡ 3feaf821-2d4d-4e6a-ba8a-8964052c0302
md"## Normalisation des modes"

# ╔═╡ 8e64314e-8cfa-43b0-9357-3c01ae2b036c
md"""
Dans l'approximation scalaire, l'intégrale de recouvrement entre deux modes est : $\iint E_1 E_2^* dS$. Elle peut être calculée avec la fonction `overlap`.
"""

# ╔═╡ 6f1b8944-367c-467f-8a97-40e0defdcc2c
md"Les modes calculés par les modes solvers ne sont pas normalisés :"

# ╔═╡ 6afef2d2-2bcc-42ed-86a0-90640bc0d41d
overlap.(ms2D,transpose(ms2D))

# ╔═╡ c1451fd4-97de-4dee-a3ce-dbd0c1907e1b
md"Les modes peuvent être normalisés à l'aide de la fonction `normalize!`"
...
md"""
## Couplage dans une fibre
Un faisceau gaussien de waist wf est injecté dans la fibre.
"""
...
md"On définit tout d'abord le faisceau injecté"
...
md"On peut vérifier qu'il est bien normalisé"

# ╔═╡ a210296a-bb34-4e40-b394-9d9691ac11fa
overlap(f,f)

# ╔═╡ 16558d1e-901a-4eae-abe6-25069f338a45
md"On calcule ensuite le recouvrement avec chaque mode :"

# ╔═╡ 293510e1-336a-47d4-99fe-18d336cabff4
ol=overlap.(ms2D,f)

# ╔═╡ f5a31503-47f4-4f0c-9354-ff2d0e762d9e
md"La fraction de puissance transmise dans la fibre est (réflexion de Fresnel négligée) :"

# ╔═╡ 9518861f-6b82-487a-96ad-2f78a1b4ede6
T=sum(abs2.(ol))

# ╔═╡ 43a98053-bace-40c8-8b3f-fae68cf2c4c3
md"La fraction de puissance transmise dans le mode fondamental est :"

# ╔═╡ 4d27b5b6-22d9-4aab-ae2f-da3209752ac9
T01=abs2(ol[1])

# ╔═╡ 78ec6f77-cb50-49e3-800e-0a26010ed8fd
md"Une formule analytique permet de retrouver ce dernier résultat. En effet, la transmission d'un mode gaussien A de waist $w_A$ vers un mode gaussien de waist $w_B$ est : $T=\frac{4w_A^2w_B^2}{\left(w_A^2+w_B^2\right)^2}$"

# ╔═╡ 0d716824-f534-4e9f-9979-0438ff4fd073
md"On peut assimiler le mode fondamental à un mode gaussien dont le waist est la moitié du diamètre de mode et calculer le recouvrement avec le faisceau gaussien de façon analytique"

# ╔═╡ cc38bd13-c243-499c-a0e4-9f7d07c5d0b2
wm=MFD(ms2D[1])/2

# ╔═╡ e82934f6-bfde-48ba-ba71-3da03b01c7d0
Tfond_analytique=4*wf^2*wm^2/(wf^2+wm^2)^2

# ╔═╡ 9f40fd9e-f902-438b-b2af-e3c105684cbc
md"""
## Propagation dans la fibre
Pour propager un mode, il suffit de multiplier le champ par $\exp(i\beta z)$
"""

# ╔═╡ 1e3cbba1-5448-4819-a6c8-de7de845981d
md"Par symétrie, les deux modes mis en jeu sont LP$_{01}$ (mode 1) et LP$_{02}$ (mode 4). La période de battement (en µm) entre ces deux modes est :"

# ╔═╡ 2cfb5868-2aae-475f-aede-1a44a4a76ac1
lambda/(ms2D[1].neff-ms2D[4].neff)

# ╔═╡ 2f6b424b-18e2-40a8-861e-6295b1f822d5
md"Distance de propagation $z$"
...
# ╔═╡ 590b3dfa-0a8d-4174-9763-5a31541fa334
begin
	x=-10:0.1:10
	p1=contourf(x,x,real(computeField(abs2(f),x,x'))',levels=100,linewidth=0,aspect_ratio=:equal,xlims=(minimum(x),maximum(x)),ylims=(minimum(x),maximum(x)),title="|E|² - Injection")
	p2=contourf(x,x,real(computeField(abs2(fz),x,x'))',levels=100,linewidth=0,aspect_ratio=:equal,xlims=(minimum(x),maximum(x)),ylims=(minimum(x),maximum(x)),title="|E|² - z = $(z)")
...
md"""
## Exercice
On injecte une gaussienne de waist $wf=2$ µm mais décentrée d'une distance $d=2.5$ µm par rapport au centre de la fibre. Déterminer la transmission en dB dans le mode fondamental et dans le premier mode d'ordre supérieur.
"""

# ╔═╡ 999985c3-9d8d-4b64-86a2-a81840f75b9e
d=2.5

# ╔═╡ 345c2ff6-a68e-467f-a05f-82cdf02c546e
...
manifest_format = "2.0"
project_hash = "c0e0f7da15c1069cbe540bd5a57d5a480ac975b3"
